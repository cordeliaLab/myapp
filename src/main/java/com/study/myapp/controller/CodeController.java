...
public class CodeController {
    ...
    // 코드 공개여부
    @RequestMapping(value = {"/codeList/setCodeUse", "/codeWrite/setCodeUse"}, method = RequestMethod.POST)
    public String setCodeUse(HttpServletRequest request,
                                CodeModel codeModel, Model model,
                                @AuthenticationPrincipal UserDetail userDetail) throws Exception {
        codeModel.setModifyUserPid(userDetail.getMember().getUserPid());
        codeModel.setCreateUserPid(userDetail.getMember().getUserPid());
        int iResult = this.codeService.setCodeUse(codeModel);
        if (iResult == 1) {
            model.addAttribute("msg", StaticMessage.message(200,"코드공개여부"));
        } else {
            model.addAttribute("msg", StaticMessage.message(0,"코드공개여부"));
        }
        return StaticData.index(request) + ":: #CommMsgData";
    }
}
